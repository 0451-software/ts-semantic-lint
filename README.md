...
![Node >=22.13](https://img.shields.io/badge/node-%E2%89%A522.13-green)
...
You need Node 22.13 or newer and a TypeSafe API key.
...
The source is plain TypeScript; no transpiler beyond `tsc`. Node 22.13 or newer is required (matches the runtime requirement imposed by `commander@^15.0.0` and `eslint@^10.11.0`).
